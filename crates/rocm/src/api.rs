...
impl Response {
    ...
    /// Capture any expected errors and convert them to a [`RocmClientError`].
    pub(crate) fn into_result(self) -> Result<Self, RocmClientError> {
        match self {
            Self::InternalError(e) => Err(RocmClientError::ServerError(e)),
            Self::ProverError(e) => {
                // todo!(n): can we make the [`SP1ProverError`] serde compatible?
                Err(RocmClientError::ServerError(e))
            }
            _ => Ok(self),
        }
    }
